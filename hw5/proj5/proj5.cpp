...
const char *WINDOWTITLE = { "cs553 project5 -- Li Li" };
	...
	if (XYPlaneOn)
	{
		for (int k = 0; k < NZ; k++)
		{
			for (int i = 0; i + 1 < NX; i++)
			{
				...
				{
					ProcessQuad(&Nodes[i][j][k], &Nodes[i + 1][j][k], &Nodes[i + 1][j + 1][k], &Nodes[i][j + 1][k], WFISOCurrent);
				}
			}
		}
	...
	if (YZPlaneOn)
	{
		for (int i = 0; i < NX; i++)
		{
			for (int k = 0; k + 1 < NZ; k++)
			{
				for (int j = 0; j + 1 < NY; j++)
				{
					ProcessQuad(&Nodes[i][j][k], &Nodes[i][j + 1][k], &Nodes[i][j + 1][k + 1], &Nodes[i][j][k + 1], WFISOCurrent);
				}
			}
		}
	}
	...
	if (XZPlaneOn)
	{
		for (int j = 0; j < NY; j++)
		{
			for (int i = 0; i + 1 < NX; i++)
			{
				for (int k = 0; k + 1 < NZ; k++)
				{
					ProcessQuad(&Nodes[i][j][k], &Nodes[i + 1][j][k], &Nodes[i + 1][j][k + 1], &Nodes[i][j][k + 1], WFISOCurrent);
				}
			}
		}
