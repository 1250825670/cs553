...
const char *WINDOWTITLE = { "cs553 project7 -- Li Li" };
...
int	WhichProjection = ORTHO;		// ORTHO or PERSP
	...
	if( ( ActiveButton & LEFT ) != 0 )
	{
		switch( LeftButton )
		{
			case ROTATE:
			{
				...
				int changeflag = (!((dx + dy > 0) ^ (Xmouse > Ymouse))) ? 1 : 0;
				Zrot += changeflag ? -(sqrt(SQR(dy) + SQR(dx))) : (sqrt(SQR(dy) + SQR(dx)));
