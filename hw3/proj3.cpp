...
const char *WINDOWTITLE = { "cs553 project3 -- Li Li" };
...
InitGlui( )
{
	...
	//add sliders for proj3
	char str[128];

	XSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, XLowHigh,
		X_ID, (GLUI_Update_CB)Sliders);
	XSlider->set_float_limits(-1.f, 1.f);
	...
	ZSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, ZLowHigh,
		Z_ID, (GLUI_Update_CB)Sliders);
	ZSlider->set_float_limits(-1., 1.);
	ZSlider->set_w(200);		// good slider width
	sprintf(str, ZFORMAT, ZLowHigh[0], ZLowHigh[1]);
	ZLabel = Glui->add_statictext(str);

	RadSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, RadLowHigh,
		RAD_ID, (GLUI_Update_CB)Sliders);
	RadSlider->set_float_limits(.0, sqrt(3.));
	RadSlider->set_w(200);		// good slider width
	sprintf(str, RadFORMAT, RadLowHigh[0], RadLowHigh[1]);
	RadLabel = Glui->add_statictext(str);

	GradSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, GradLowHigh,
		GRAD_ID, (GLUI_Update_CB)Sliders);
	GradSlider->set_float_limits(.0, 300.);
	...
	TempSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, TempLowHigh,
		TEMP_ID, (GLUI_Update_CB)Sliders);
