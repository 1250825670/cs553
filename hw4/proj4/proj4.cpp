...
const char *WINDOWTITLE = { "cs553 project4 -- Li Li" };
...
//sliders variables for proj4
// XY, YZ, and XZ
struct node  XYPlane[NX][NY];
struct node  YZPlane[NY][NZ];
struct node  XZPlane[NX][NZ];

int	AlldataOn;				// != 0 means to displayalldata
int SmoothColorOn;			// !=0 means using smooth color shading otherwise using contour lines
int	XYPlaneOn;				// != 0 means to displayXYPlane
int	YZPlaneOn;				// != 0 means to displayYZPlane
int	XZPlaneOn;				// != 0 means to displayXZPlane

const char *		XYFORMAT = { "XY Plane: %d" };
int			XYCurrent = 5;
GLUI_HSlider *		XYSlider;
GLUI_StaticText *	XYLabel;

const char *		YZFORMAT = { "YZ Plane: %d" };
int			YZCurrent = 5;
GLUI_HSlider *		YZSlider;
GLUI_StaticText *	YZLabel;

const char *		XZFORMAT = { "XZ Plane: %d" };
int			XZCurrent = 5;
GLUI_HSlider *		XZSlider;
GLUI_StaticText *	XZLabel;

enum slider_id { XY_ID, YZ_ID, XZ_ID };
...
Sliders(int id)
{
	...
	case XY_ID:
		sprintf(str, XYFORMAT, XYCurrent);
		XYLabel->set_text(str);
		break;

	case YZ_ID:
		sprintf(str, YZFORMAT, YZCurrent);
		YZLabel->set_text(str);
		break;

	case XZ_ID:
		sprintf(str, XZFORMAT, XZCurrent);
...
int NodeIntersectCheck(struct node *p0, struct node *p1, struct node *p, float Scalar)
{
	//check S0 == S1
	if (p0->t == p1->t)
	{
		//whether this point equals to our Scalar or not we do nothing
		return 0;
	}
	else
	{
		float t = (Scalar - p0->t) / (p1->t - p0->t);
		if (t <= 1. && t >= 0.)
		{
			p->x = p0->x + t * (p1->x -p0->x);
			p->y = p0->y + t * (p1->y - p0->y);
			p->z = p0->z + t * (p1->z - p0->z);
			p->rgb[0] = p0->rgb[0] + t * (p1->rgb[0] - p0->rgb[0]);
			p->rgb[1] = p0->rgb[1] + t * (p1->rgb[1] - p0->rgb[1]);
			p->rgb[2] = p0->rgb[2] + t * (p1->rgb[2] - p0->rgb[2]);
			return 1;//Intersect
		}
		else
			return 0;
...
void
ProcessQuad(struct node *p0, struct node *p1, struct node *p2, struct node *p3, float Scalar)
{
	int CrossCount = 0;
	struct node p[4];

	//check p0-p1
	CrossCount += NodeIntersectCheck(p0, p1, &p[CrossCount], Scalar);
	//check p1-p2
	CrossCount += NodeIntersectCheck(p1, p2, &p[CrossCount], Scalar);
	//check p2-p3
	CrossCount += NodeIntersectCheck(p2, p3, &p[CrossCount], Scalar);
	//check p3-p0
	CrossCount += NodeIntersectCheck(p3, p0, &p[CrossCount], Scalar);
	float M;
	switch (CrossCount)
	{
	case 0:
		...
		break;
	case 1:
		fprintf(stderr, " %d intersect(s)\n",CrossCount);
		break;
	case 2:
		//normal two intersects draw it

		glColor3f(p[0].rgb[0], p[0].rgb[1], p[0].rgb[2]);
	...
	case 3:
		...
		break;
	case 4:
		// linearly-interpolate the Scalar
		M = .25*p0->t + .25*p1->t + .25*p2->t + .25*p3->t;

		//1. check if Scalar is near the top or bot

		//2. need to determine nodes scalar value sequence
		//	given 4 nodes, 2 lower as L and 2 higher as H
		//	there are only types: LHLH or HLHL
		
		if (((Scalar > M) && (p0->t < p1->t)) 
			|| ((Scalar <= M) && (p0->t >= p1->t)))
		{
				// S>M and LHLH
				// or M<=S and HLHL
				...
				glVertex3f(p[0].x, p[0].y, p[0].z);
				glVertex3f(p[1].x, p[1].y, p[1].z);
				glColor3f(p[2].rgb[0], p[2].rgb[1], p[2].rgb[2]);
		...
		else
		{
			...
			glVertex3f(p[0].x, p[0].y, p[0].z);
			glVertex3f(p[3].x, p[3].y, p[3].z);
			glColor3f(p[1].rgb[0], p[1].rgb[1], p[1].rgb[2]);
			glVertex3f(p[1].x, p[1].y, p[1].z);
			glVertex3f(p[2].x, p[2].y, p[2].z);
		}
	...
	if (XYPlaneOn)
	{
		for (int i = 0; i < NX; i++)
			{
				for (int j = 0; j < NY; j++)
				{
					XYPlane[i][j] = Nodes[i][j][XYCurrent];
				}
			}
		...
		if (SmoothColorOn)
		{
			...
			for (int i = 0; i+1 < NX; i++)
			{
				...
				{
					glBegin(GL_QUADS);
					glColor3f(XYPlane[i][j].rgb[0], XYPlane[i][j].rgb[1], XYPlane[i][j].rgb[2]);
					glVertex3f(XYPlane[i][j].x, XYPlane[i][j].y, XYPlane[i][j].z);

					glColor3f(XYPlane[i + 1][j].rgb[0], XYPlane[i + 1][j].rgb[1], XYPlane[i + 1][j].rgb[2]);
					glVertex3f(XYPlane[i + 1][j].x, XYPlane[i + 1][j].y, XYPlane[i + 1][j].z);

					glColor3f(XYPlane[i + 1][j + 1].rgb[0], XYPlane[i + 1][j + 1].rgb[1], XYPlane[i + 1][j + 1].rgb[2]);
					glVertex3f(XYPlane[i + 1][j + 1].x, XYPlane[i + 1][j + 1].y, XYPlane[i + 1][j + 1].z);

					glColor3f(XYPlane[i][j + 1].rgb[0], XYPlane[i][j + 1].rgb[1], XYPlane[i][j + 1].rgb[2]);
					glVertex3f(XYPlane[i][j + 1].x, XYPlane[i][j + 1].y, XYPlane[i][j + 1].z);
					glEnd();
				}
			}
		}
		...
		{
			// Compute contour distribution
			int numS = 20;
			int iS;
			float SCur;
			float Smin = 0.0;
			float Smax = 100.;
			for (iS = 0, SCur = Smin; iS < numS; iS++, SCur += ((Smax - Smin) / 12.))
			{
				glBegin(GL_LINES);
				for (int i = 0; i + 1 < NX; i++)
				{
					for (int j = 0; j + 1 < NY; j++)
					{
						ProcessQuad(&XYPlane[i][j], &XYPlane[i + 1][j], &XYPlane[i + 1][j + 1], &XYPlane[i][j+1], SCur);
					}
				}
				glEnd();
			}
	...
	if (YZPlaneOn)
	{
		for (int i = 0; i < NY; i++)
			{
				for (int j = 0; j < NZ; j++)
				{
					YZPlane[i][j] = Nodes[YZCurrent][i][j];
				}
			}
		...
		if (SmoothColorOn)
		{
			...
			for (int i = 0; i+1 < NY; i++)
			{
				...
				{
					glBegin(GL_QUADS);
					glColor3f(YZPlane[i][j].rgb[0], YZPlane[i][j].rgb[1], YZPlane[i][j].rgb[2]);
					glVertex3f(YZPlane[i][j].x, YZPlane[i][j].y, YZPlane[i][j].z);

					glColor3f(YZPlane[i + 1][j].rgb[0], YZPlane[i + 1][j].rgb[1], YZPlane[i + 1][j].rgb[2]);
					glVertex3f(YZPlane[i + 1][j].x, YZPlane[i + 1][j].y, YZPlane[i + 1][j].z);

					glColor3f(YZPlane[i + 1][j + 1].rgb[0], YZPlane[i + 1][j + 1].rgb[1], YZPlane[i + 1][j + 1].rgb[2]);
					glVertex3f(YZPlane[i + 1][j + 1].x, YZPlane[i + 1][j + 1].y, YZPlane[i + 1][j + 1].z);

					glColor3f(YZPlane[i][j + 1].rgb[0], YZPlane[i][j + 1].rgb[1], YZPlane[i][j + 1].rgb[2]);
					glVertex3f(YZPlane[i][j + 1].x, YZPlane[i][j + 1].y, YZPlane[i][j + 1].z);
					glEnd();
				}
			}
		}
		else
		{
			...
			for (iS = 0, SCur = Smin; iS < numS; iS++, SCur += ((Smax - Smin) / 12.))
			{
				...
				for (int i = 0; i + 1 < NY; i++)
				{
					for (int j = 0; j + 1 < NZ; j++)
					{
						ProcessQuad(&YZPlane[i][j], &YZPlane[i + 1][j], &YZPlane[i + 1][j + 1], &YZPlane[i][j+1], SCur);
					}
				}
				glEnd();
			}
	...
	if (XZPlaneOn)
	{
		for (int i = 0; i < NX; i++)
			{
				for (int j = 0; j < NZ; j++)
				{
					XZPlane[i][j] = Nodes[i][XZCurrent][j];
				}
			}
		...
		if (SmoothColorOn)
		{
			...
			for (int i = 0; i+1 < NX; i++)
			{
				...
				{
					glBegin(GL_QUADS);
					glColor3f(XZPlane[i][j].rgb[0], XZPlane[i][j].rgb[1], XZPlane[i][j].rgb[2]);
					glVertex3f(XZPlane[i][j].x, XZPlane[i][j].y, XZPlane[i][j].z);

					glColor3f(XZPlane[i + 1][j].rgb[0], XZPlane[i + 1][j].rgb[1], XZPlane[i + 1][j].rgb[2]);
					glVertex3f(XZPlane[i + 1][j].x, XZPlane[i + 1][j].y, XZPlane[i + 1][j].z);

					glColor3f(XZPlane[i + 1][j + 1].rgb[0], XZPlane[i + 1][j + 1].rgb[1], XZPlane[i + 1][j + 1].rgb[2]);
					glVertex3f(XZPlane[i + 1][j + 1].x, XZPlane[i + 1][j + 1].y, XZPlane[i + 1][j + 1].z);

					glColor3f(XZPlane[i][j + 1].rgb[0], XZPlane[i][j + 1].rgb[1], XZPlane[i][j + 1].rgb[2]);
					glVertex3f(XZPlane[i][j + 1].x, XZPlane[i][j + 1].y, XZPlane[i][j + 1].z);
					glEnd();
				}
			}
		}
		else
		{
			...
			for (iS = 0, SCur = Smin; iS < numS; iS++, SCur += ((Smax - Smin) / 12.))
			{
				...
				for (int i = 0; i + 1 < NX; i++)
				{
					for (int j = 0; j + 1 < NZ; j++)
					{
						ProcessQuad(&XZPlane[i][j], &XZPlane[i + 1][j], &XZPlane[i + 1][j + 1], &XZPlane[i][j+1], SCur);
					}
				}
				glEnd();
			}
...
InitGlui( )
{
	...
	Glui->add_checkbox("Show all data", &AlldataOn);

	Glui->add_checkbox("Using smooth shading/contour lines", &SmoothColorOn);

	Glui->add_checkbox("XY Plane", &XYPlaneOn);
	Glui->add_checkbox("YZ Plane", &YZPlaneOn);
	Glui->add_checkbox("XZ Plane", &XZPlaneOn);
	...
	XYSlider = Glui->add_slider(false, GLUI_HSLIDER_INT, &XYCurrent,
		XY_ID, (GLUI_Update_CB)Sliders);
	XYSlider->set_int_limits(0, NZ-1);
	XYSlider->set_w(200);		// good slider width
	sprintf(str, XYFORMAT, XYCurrent);
	XYLabel = Glui->add_statictext(str);

	YZSlider = Glui->add_slider(false, GLUI_HSLIDER_INT, &YZCurrent,
		YZ_ID, (GLUI_Update_CB)Sliders);
	YZSlider->set_int_limits(0, NX - 1);
	YZSlider->set_w(200);		// good slider width
	sprintf(str, YZFORMAT, YZCurrent);
	YZLabel = Glui->add_statictext(str);

	XZSlider = Glui->add_slider(false, GLUI_HSLIDER_INT, &XZCurrent,
		XZ_ID, (GLUI_Update_CB)Sliders);
	XZSlider->set_int_limits(0, NY - 1);
	XZSlider->set_w(200);		// good slider width
	sprintf(str, XZFORMAT, XZCurrent);
	XZLabel = Glui->add_statictext(str);


	//spinner point size;
	GLUI_Spinner *pointsize_spinner = Glui->add_spinner("PointSize",
				GLUI_SPINNER_FLOAT,
				&PointSize, NULL,
				(GLUI_Update_CB) SetPointSize);
	pointsize_spinner->set_float_limits(1., 10., GLUI_LIMIT_CLAMP);
